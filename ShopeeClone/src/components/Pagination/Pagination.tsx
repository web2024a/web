    ...
    return Array(pageSize)
      ...
      .map((_, index) => {
        ...
        if (page <= RANGE * 2 + 1 && pageNumber > page + RANGE && pageNumber < pageSize - RANGE + 1) {
          ...
          } else if (pageNumber > page + RANGE && page < pageSize - RANGE + 1) {
            return renderDotAfter(index)
          }
        ...
        return (
          <Link
            ...
            key={index}
            className={`bg-white rounded px-3 py-2 shadow-sm mx-2 cursor-pointer border ${pageNumber === page ? 'border-cyan-500' : 'border-transparent'}`}
          >
            {pageNumber}
          </Link>
        )
